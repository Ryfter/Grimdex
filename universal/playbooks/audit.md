...
- **Model provenance** (`universal/model-catalog.md` vs each `projects/*/model-usage.md`):
  - *Catch misses (mechanical, auto):* any `runner/model` in a usage log but absent from
    the catalog — e.g. a tool that didn't self-stamp at closeout (law #8) — gets
    registered (`current` for frontier; a bottom-up row for local). Update local `last-seen`.
  - *Mark supersession + flag (semantic, flag-only):* mark the prior catalog entry
    `superseded → <successor>` when a newer version has landed (the human's call, made
    here), then run
    `pwsh -NoProfile -c ". scripts/model-lib.ps1; Find-GrimdexStaleModelUsage -GrimdexRoot ."`
    — it deterministically lists usage steps still on a superseded model. Raise each as an
    OPEN CONCERN ("review whether to revisit"). Never edit the superseded work — flagging
    is the whole action.
